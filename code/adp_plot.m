...
for iset = 1 : nset
    ...
    cell_list_ns = find(~set{iset, 1}.vis_driven_ad);
...
dfof_equiv_ad_targ_ns = []; dfof_equiv_ad_ns = []; adp_ratio_a0t0_ns = [];
for ii = 1 : length(cell_list_ns)
    icell_ns = cell_list_ns(ii);
    idelta = 8; % targ0 only! adp is ori-specific
    for igap = 1:ngap
        dfof_equiv_ad_targ_ns(ii, igap) = set{iset, 2}.dfof_avg_merge(icell_ns, idelta, igap+1); %750-250
        dfof_equiv_ad_ns(ii, igap) = dfof_avg_ad(icell_ns, idelta);
        adp_ratio_a0t0_ns(ii, 1, igap) = dfof_equiv_ad_targ_ns(ii, igap) / dfof_equiv_ad_ns(ii, igap) - 1;
    end
end
adp_sig(iset).dfof_equiv_ad_ns = dfof_equiv_ad_ns(:,1);
adp_sig(iset).dfof_equiv_ad = dfof_equiv_ad(:,1);
adp_sig(iset).adp_vis_driven = squeeze(adp_ratio_a0t0);
adp_sig(iset).adp_ns = squeeze(adp_ratio_a0t0_ns);
...
low_ad_resp{iset} = logical(abs(dfof_equiv_ad(:,1))<0.01...
    | abs(adp_ratio_a0t0(:, 1, 1))>2 | abs(adp_ratio_a0t0(:, 1, 2))>2); % w hard threshold to evict posthoc outliers
...
ylim(hAx,[-2, 2])
% saveas(gcf, ['adp ratio a0t0 across area-mouse'], 'jpg'); close 
...
for igap = 1 : ngap
    ...
    for iarea = 1 : narea
        ...
        for iset = 1 : length(area_set_seq)
            trace_area_cat = [trace_area_cat; trace(area_set_seq(iset)).trace_set{igap,1}];
            ncell_area(iarea) = ncell_area(iarea) + trace(area_set_seq(iset)).ncell;
        end
        trace_area_avg(iarea,:) = nanmean(trace_area_cat,1);
        trace_area_std(iarea,:) = nanstd(trace_area_cat,1);
        trace_area_sem(iarea,:) = trace_area_std(iarea,:) ./ sqrt(ncell_area(iarea));
    end

    trace_len = 3.5 * 30; % trace len 3.5 s according to Jin2019 Fig 2B
    color_list = {[0,0,1], [0,1,0], [1,0,0]};
    for iarea = 1 : narea
        plot(1:trace_len, trace_area_avg(iarea, 1:trace_len), 'color',color_list{iarea}, 'LineWidth', 2); hold on
        xlim([0, 105])
    end
    
    for iarea = 1 : narea
        x = 1:trace_len; x2 = [x, fliplr(x)];
        curve1 = trace_area_avg(iarea, 1:trace_len) + trace_area_sem(iarea, 1:trace_len); curve2 = trace_area_avg(iarea, 1:trace_len) - trace_area_sem(iarea, 1:trace_len);
%         curve1 = trace_area_avg(iarea, 1:trace_len) + trace_area_std(iarea, 1:trace_len); curve2 = trace_area_avg(iarea, 1:trace_len) - trace_area_std(iarea, 1:trace_len);
    ...
    legend(['V1 (' num2str(ncell_area(1)) ')'], ['LM (' num2str(ncell_area(2)) ')'], ['LI (' num2str(ncell_area(3)) ')'],...
        'Location','northeast'); legend boxoff
    title([isi_str{igap}])
end

ylim(hAx,[-0.03, 0.22])
% saveas(gcf, ['trace across area w sem'], 'jpg'); close
