...
dataset_list = struct;
dataset_list.mouse = [1322];
dataset_list.date = [200803];
dataset_list.area = {'V1'};
dataset_list.areacode = {1};
...
for iset = 1 : nset
    ...
    areamousedate = [area '_' imouse '_' date '_py'];
    result_prefix = 'C:\Users\lan\Documents\repos\inter\mat\';
    result_folder{iset} = fullfile(result_prefix, areamousedate);
end

iset=2;
result_folder{2} = 'C:\Users\lan\Documents\repos\inter\mat\V1_i1322_200803';
...
% save adp_idx.mat adp_a0t0_merge
...
% saveas(gcf, ['adp movmean movstd across dfof_ad & scatter'], 'jpg'); 
close
...
ylabel('adp moving std'); ylim([0,2])
saveas(gcf, ['adp movmean movstd across dfof_ad all'], 'jpg'); 
% close
