...
%% response to 8 ori targets. ori-driven cell
% dfof_ad = ncell x 1. dfof_tg = ncell x nori x nisi

sig_alpha = 0.01; resp_threshold = 0.1;
[dfof_tg, dfof_tg_sem, dfof_tg_std, ori_driven] = dfof_tg_ori_driven_ohki(dfof_align_tg, sig_alpha, resp_threshold);
if save_flag; save dfof_tg.mat dfof_tg dfof_tg_sem dfof_tg_std; end 

%% visually-driven cell
% cells responsive to ad / noad tg (all oris)

[vis_driven, ~] = vis_driven_ohki(dfof_align_tg, sig_alpha);
% find(vis_cell_noad_tg==0) % not vis driven by noad tg
if save_flag; save cell_property.mat ori_driven vis_driven; end
