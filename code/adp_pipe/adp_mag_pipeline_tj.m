...
mat_dir = 'Z:\All_Staff\home\tj\Analysis\2P\210904_i1350\210904_i1350_runs-001'
fn_base = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff';
user_fn = fullfile(fn_base, 'home\tj'); 
...
dataset_list.date = [210904];
dataset_list.area = {'V1'};

%% load stim input & time course
...
frame_rate = 15.5 % see exp record Z:\All_Staff\home\tj\2p_Imaging\i1350\210904_i1350

cd(mat_dir)
input_file = dir('*input*.mat').name
temp = load(input_file); % load behavior data "input"
input_behav = temp.input; clear temp

TC_file = dir('*TC*.mat').name
df = load(TC_file);
npSub_tc = df.npSub_tc; % shape = [nframe, ncell]
size(npSub_tc)

areamousedate = [area '_' imouse '_' date];
result_folder = [root_path, '\mat\', areamousedate, '_TJ'];
...
% 464 = 16 dir * ? adapter contrast * 2 ISI * 14.5 reps
