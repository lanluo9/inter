...
def calc_trace_stim(trace_by_trial, stim_id):
    ...
    trace_cell_avg = np.mean(
        np.mean(trace_by_trial, axis=0), axis=0
    )  # trace of avg cell over all stims

    trace_stim_avg = []
    for i in np.unique(stim_id):
        trace_istim_avg = np.mean(
            trace_by_trial[:, np.where(stim_id == i)[0]], axis=1
        )  # ncell x nframe
        trace_istim_avg = np.mean(trace_istim_avg, axis=0)  # nframe
        trace_stim_avg.append(trace_istim_avg)
    print(len(trace_stim_avg), trace_stim_avg[0].shape)  # trace for each stim
