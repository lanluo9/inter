...
ax = sns.catplot(x="area", y="adp_mag", col="isi",
                data=df_adp_mag_thres, kind="violin", split=True,
...
ax = sns.catplot(x="area", y="adp_mag", col="isi",
                data=df_adp_mag_thres, kind="strip",
                height=4, aspect=1)
ax.set(ylabel = 'adaptation index');
...
g = sns.catplot(x="area", y="adp_mag", hue="isi", 
                data=df_adp_mag_thres, kind="point",
                dodge=True, height=5, aspect=1.4);

# ax = sns.catplot(x="area", y="adp_mag", col="isi",
#                 data=df_adp_mag_thres, kind="strip",
#                 height=4, aspect=1.2);

g.map_dataframe(sns.stripplot, x="area", y="adp_mag", #data=df_adp_mag_thres,
                palette='pastel', alpha=0.9, dodge=True)
# g.map(sns.stripplot, "area", "adp_mag",
...
df_adp_mag_thres_250 = df_adp_mag_thres[df_adp_mag_thres.isi==250]
...
ax = sns.violinplot(x="area", y="adp_mag", data=df_adp_mag_thres_250,
                inner=None, color=".9");
ax = sns.stripplot(x="area", y="adp_mag", data=df_adp_mag_thres_250, alpha=0.8);


# In[107]:


ax = sns.catplot(x="area", y="adp_mag", hue="isi", 
                data=df_adp_mag_thres, kind="point",
                dodge=True, height=5, aspect=1.4);
...
ax = sns.catplot(x="area", y="adp_mag", 
                data=df_adp_mag_thres_250, kind="violin", 
                height=4, aspect=1.6);
ax = sns.catplot(x="area", y="adp_mag",
                data=df_adp_mag_thres_250, kind="strip",
                height=4, aspect=1.6);
...
def swap_df_rows(df, irow):
    # irow = [[pair1, pair1], [pair2, pair2], ...]
    # df2 = swap_df_rows(df, irow=[[1,2],[3,4],[5,6]])
    
    npair = np.asarray(irow).shape[0]
    
    for i in np.arange(npair):
        rowA = np.asarray(irow)[i,0]; rowB = np.asarray(irow)[i,1]
        b, c = df.iloc[rowA].copy(), df.iloc[rowB].copy() 
        df.iloc[rowA], df.iloc[rowB] = c, b
...
b, c = df_adp_dist_mag.iloc[0].copy(), df_adp_dist_mag.iloc[1].copy() 
df_adp_dist_mag.iloc[0], df_adp_dist_mag.iloc[1] = c, b
b, c = df_adp_dist_mag.iloc[2].copy(), df_adp_dist_mag.iloc[4].copy() 
df_adp_dist_mag.iloc[2], df_adp_dist_mag.iloc[4] = c, b# swap row 0 & 1 to sort df.gb properly
...
ax = sns.lineplot(data=tt[tt.area == 'V1'], x="ori_pref_bin", y="adp_mag", hue="area", style="isi");
