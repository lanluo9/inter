...
data_dates = df_now.date.unique()
nset = len(data_dates) # multisess 1 date = 1 dataset, already merged tiff of each session in matlab adp_mag_mix50.m
...
for i in np.arange(nset):
    ...
    mouse = df_now.loc[df.date == data_dates[i]].iloc[0].mouse.astype(str)
    imouse = 'i' + mouse
    date = df_now.loc[df.date == data_dates[i]].iloc[0].date.astype(str)
...
# df = data_info.copy()
# df_stim = df[df['stim_type'] == 'grating']

# nset = df_stim.shape[0]
...
#     clear_output(wait=True)
#     print('set ', i+1, 'of', nset)
#     mouse = df_stim.iloc[i].mouse.astype(str)
#     imouse = 'i' + mouse
#     date = df_stim.iloc[i].date.astype(str)
#     irun = '00' + df_stim.iloc[i].num.astype(int).astype(str)
#     print(imouse, date, irun)
