...
f_oneway(df_th_dropna[df_th_dropna.stim==0].mag, df_th_dropna[df_th_dropna.stim==1].mag, df_th_dropna[df_th_dropna.stim==2].mag,          df_th_dropna[df_th_dropna.stim==3].mag, df_th_dropna[df_th_dropna.stim==4].mag,          df_th_dropna[df_th_dropna.stim==5].mag, df_th_dropna[df_th_dropna.stim==6].mag)
...
#              color='blue', linewidth=3, capsize=3, capthick=3, alpha=0.8);
# plt.legend(['median', 'mean'])
plt.text(0.05, 0.15, 'one-way anova p value = 0.007 \nn=121', transform=plt.gca().transAxes, fontsize=18, verticalalignment='top')

plt.ylabel('Adaptation index', fontsize=18);
plt.xlabel('Stimulus identity sorted by adaptation index', fontsize=18);
plt.xticks(fontsize=16)
plt.yticks(fontsize=16);

# os.chdir(r'C:\Users\ll357\Documents\inter\results\poster SFN 2022'.replace('\\','/'))
# plt.savefig('adp across bunny7 imgs.pdf', bbox_inches='tight')

fig4a_dict = {'mag_median_sorted': mag_median_sorted, 'mag_sem_sorted_med': mag_sem_sorted_med, 
              'nori': nori,}
import pickle
with open('fig4a_dict.pickle', 'wb') as handle:
    pickle.dump(fig4a_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
