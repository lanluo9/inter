...
mousedir = r'\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\lan\Data\2P_images\mat_inter\mix50_data'.replace('\\', '/')
...
print(out_tbt.shape) # 15 sec. ncell x ntrial x nframe
...
for subdir in os.listdir(mousedir):
    data_dir = os.path.join(mousedir, subdir)
    print(data_dir)
...
if cell_filter_name == 'img_driven': # only use img driven cells
    ...
    # data_dir_merge = data_dir.split('\\')[:-1]
    # data_dir_merge = os.path.join(*data_dir_merge)
    data_dir_merge = mousedir.replace('\\', '/')
    pref_filter = np.load(data_dir_merge + "/pref_filter.npy") # load pref_filter.npy, constructed under img driven filter. TODO: rearrange code so it can run thru
...
seed = 42
R1_agg_stim = np.array([trial_rep_select(i, 25, seed=seed).sum(axis=1) for i in resp_ad_by_stim]) # sum over 20 trial reps
R2_agg_stim = np.array([trial_rep_select(i, 25, seed=seed).sum(axis=1) for i in resp_tg_by_stim])
...
print(f'median natural: {np.nanmedian(adp_stim[:30, :].ravel())}, \n median grat: {np.nanmedian(adp_stim[30:40, :].ravel())}, \n median noise: {np.nanmedian(adp_stim[40:, :].ravel())}')
...
# df_adp_stim_type['cell_filter_name'] = 'pref' # img_driven. TODO: refactor this
df_adp_stim_type
...
# #### boot adp by stim type, sig test

# In[69]:


# ttest_ind for adaptation to grat vs nat

from scipy.stats import ttest_ind 
# test for the null hypothesis that 2 independent samples have identical average (expected) values. 

t, p = ttest_ind(df_adp_stim_type[df_adp_stim_type.stim_type == 'grating'].adp, 
                df_adp_stim_type[df_adp_stim_type.stim_type == 'natural'].adp, 
...
# one way anova for adp to grat vs nat vs noise
from scipy.stats import f_oneway
f, p = f_oneway(df_adp_stim_type[df_adp_stim_type.stim_type == 'grating'].adp,
                df_adp_stim_type[df_adp_stim_type.stim_type == 'natural'].adp,
                df_adp_stim_type[df_adp_stim_type.stim_type == 'noise'].adp)
print("f_oneway:            f = %g  p = %g" % (f, p))
...
ax = sns.violinplot(x="stim_type", y="adp",
                data=df_adp_stim_type[df_adp_stim_type.stim_type != 'noise'], kind="violin", height=4, aspect=1, linewidth=2,
...
x1, x2 = 0, 1
y, h, col = np.max(df_adp_stim_type[df_adp_stim_type.stim_type != 'noise'].adp.values) + 0.5, 0.3, 'k'
plt.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c=col)
plt.text((x1+x2)*.5, y+h+0.2, "* p=0.02", ha='center', va='bottom', color=col, fontsize=14)

# text annotation
plt.text(0.3, 0.03, f'n={df_adp_stim_grat.dropna().shape[0]}', ha='center', va='bottom', transform=ax.transAxes, fontsize=14)
plt.text(0.85, 0.03, f'n={df_adp_stim_nat.dropna().shape[0]}', ha='center', va='bottom', transform=ax.transAxes, fontsize=14)
...
for i in np.arange(len(stim_type_dict.keys())-1): # only plot natural and grat
    ...
    legend_list = ['Natural image', 'Grating', ]
    plt.plot(trace_type_avg[i], alpha=0.5, linewidth=3, label=legend_list[i])
    errbar = trace_type_sem[i]
    plt.fill_between(np.arange(len(trace_type_avg[i])), 
                 trace_type_avg[i] + errbar, trace_type_avg[i] - errbar, 
                 alpha=0.1, label='')
...
# text annotation
plt.text(0.31, 0.03, f'n={df_adp_stim_grat.dropna().shape[0]}', ha='center', va='bottom', transform=ax.transAxes, fontsize=14)
plt.text(0.85, 0.03, f'n={df_adp_stim_nat.dropna().shape[0]}', ha='center', va='bottom', transform=ax.transAxes, fontsize=14)

plt.xlabel('')
plt.ylabel('Adaptation index', fontsize=18)
ax.set_xticklabels(['Gratings', 'Natural images'], fontsize=18)
ax.tick_params(axis="x", labelsize=16)
ax.tick_params(axis="y", labelsize=16)
sns.despine(ax=ax)
plt.tight_layout()
...
def plot_adp_by_SF_grat(cell_filter, adp_stim_pop_mean, adp_stim_pop_CI95, adp_stim_pop_err):
    ...
    fontsize = 18
    ax1.set_xlabel('Preferred spatial frequency (cpd)', fontsize=fontsize)
...
# import pickle
# fig5_dict = {'cell_filter_mod': cell_filter_mod,
#             'adp_stim_pop_mean': adp_stim_pop_mean,
#             'adp_stim_pop_CI95': adp_stim_pop_CI95,
#             'adp_stim_pop_err': adp_stim_pop_err}
# with open('fig5_dict.pckl', 'wb') as f:
#     pickle.dump(fig5_dict, f)


# In[31]:


np.sum(cell_filter_mod[:, 30:38], axis=0), adp_stim_boot.shape, adp_stim_boot_pop.shape
...
adp_pop_SF_grat = adp_stim_boot_pop[:, 30:38].T
print(adp_pop_SF_grat.shape) # [nboot x nstim], pop adp across boot & SF grat
adp_cell_SF_grat = np.nanmean(adp_stim_boot, axis=0)[30:38, :]
print(adp_cell_SF_grat.shape) # [nstim x ncell], single cell adp across SF grat

from scipy.stats import f_oneway
f_oneway([*adp_pop_SF_grat][0], [*adp_pop_SF_grat][1], [*adp_pop_SF_grat][2], [*adp_pop_SF_grat][3], [*adp_pop_SF_grat][4], [*adp_pop_SF_grat][5], [*adp_pop_SF_grat][6], [*adp_pop_SF_grat][7])


# In[63]:


f_oneway(adp_cell_SF_grat[0, ~np.isnan(adp_cell_SF_grat[0])], adp_cell_SF_grat[1, ~np.isnan(adp_cell_SF_grat[1])], adp_cell_SF_grat[2, ~np.isnan(adp_cell_SF_grat[2])], adp_cell_SF_grat[3, ~np.isnan(adp_cell_SF_grat[3])], adp_cell_SF_grat[4, ~np.isnan(adp_cell_SF_grat[4])], adp_cell_SF_grat[5, ~np.isnan(adp_cell_SF_grat[5])], adp_cell_SF_grat[6, ~np.isnan(adp_cell_SF_grat[6])], adp_cell_SF_grat[7, ~np.isnan(adp_cell_SF_grat[7])])
