function [outs,stack]=stackRegister_MA_LL(stack,target,usFacs,shifts_xy)
...
for index = 1:nframes
    ...
    log_every_nframe = floor(nframes ./ 20); % log only 20 times per .sbx file
    if mod(index, log_every_nframe)==0 
        fprintf(1,'Frame %i\n',index);
    end
