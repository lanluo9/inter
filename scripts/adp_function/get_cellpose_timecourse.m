function npSub_tc = get_cellpose_timecourse(data_reg, ...
    tif_name, LL_base, arg_date, imouse, run_str, ipart)
...
if max(mask_cell(:)) > 0
    ...
    %     try
        ...
        save(fullfile(LL_base, 'Analysis\2P', [date '_' imouse], ...
            ...
            [date '_' imouse '_' run_str '_' num2str(ipart) '_TCs_cellpose.mat']), ...
            'data_tc', 'np_tc', 'npSub_tc', 'mask_cell', 'mask_np')
        clear data_tc data_tc_down np_tc np_tc_down mask_np mask_cell
        disp(['TC extraction complete for ', run_str, ' part', num2str(ipart)])
        
        % catch
        %     disp('out of memory atm or bugged out, retrying in a bit')
        %     pause(60)
        % end
    % end
